...
First, a model provider {{set-up-a-provider}}. Anthropic, IBM LiteLLM,
and OpenAI are supported today — Google coming soon. Connect via OAuth
or API key. Paste {{token-input}}, test {{test}}, save {{save}}.

Then — connections {{set-up-connections}}. OAuth apps, MCP servers,
custom secrets — every external surface your agents reach, all in
one place. We've already linked GitHub here — we'll need it in a
minute {{agents}}.
...
Now real work. {{prompt-input}} Check the Dam repository — who
changed what this week, write me a markdown digest, save it. Send
{{send}}, and wait for the agent's result.
...
reachable from your phone — or anyone on the team you've shared
access with.

Generated files land right here. Open one {{app-js}} — the digest,
right in the panel. Edit it, or add your own. The workspace is a
volume that outlives the pod — your edits are there the next time
the agent wakes up.
